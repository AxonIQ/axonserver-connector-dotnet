...
public class AxonServerConnectionFactoryOptionsTests
{
    ...
    {
        IEnumerable<KeyValuePair<string,string>> tags = _fixture.CreateMany<KeyValuePair<string, string>>(Random.Shared.Next(1,5)).ToArray();
    ...
    [Fact]
    public void FromConfigurationRequiresComponentName()
    {
        Assert.Throws<InvalidOperationException>(
            () => AxonServerConnectionFactoryOptions.FromConfiguration(
    ...
    public void FromMinimalConfigurationReturnsExpectedResult()
    {
        ...
        var configuration = new ConfigurationRoot(new List<IConfigurationProvider> { new MemoryConfigurationProvider(source) });
