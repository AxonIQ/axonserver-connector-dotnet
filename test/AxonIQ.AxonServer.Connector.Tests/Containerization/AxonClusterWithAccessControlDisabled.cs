...
{
    public AxonClusterWithAccessControlDisabled(IMessageSink logger)
    {
        if (logger == null) throw new ArgumentNullException(nameof(logger));
        logger.OnMessage(new DiagnosticMessage("Using Embedded Axon Server Container outside of CI"));
        Cluster = EmbeddedAxonCluster.WithAccessControlDisabled(new MessageSinkLogger<EmbeddedAxonCluster>(logger));
    }
